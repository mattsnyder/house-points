              ...
              <div class="p-8">
                    ...
                    <%= for {house_data, index} <- Enum.with_index(@house_totals) do %>
                      <div class={"relative flex items-center p-6 rounded-xl border-2 shadow-lg #{house_border_class(house_data.house_name, index)} transform hover:scale-[1.02] transition-transform duration-200"}>
                        <div class="flex-shrink-0 w-12 text-center">
                          <%= case index do
                            0 -> ~H"<span class='text-3xl'>🥇</span>"
                            1 -> ~H"<span class='text-3xl'>🥈</span>"
                            2 -> ~H"<span class='text-3xl'>🥉</span>"
                            _ -> ~H"<span class='text-xl font-bold text-base-content/60'>#{index + 1}</span>"
                          end %>
                        </div>
                ...
                <% else %>
                  <div class="text-center py-12">
                    ...
                    <p class="text-gray-300 text-lg mb-6">Begin the house championship by recognizing exceptional magical achievements!</p>
